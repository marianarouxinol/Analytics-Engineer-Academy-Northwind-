with
    ...
    , joined as (
        select
            order_itens.order_item_sk
            , order_itens.product_fk
            , orders.employee_fk
            , orders.customer_fk
            , orders.shipper_fk
            , orders.order_date
        ...
        from order_itens
        inner join orders on order_itens.order_fk = orders.order_pk
    )
    , metrics as (
        select
            order_item_sk
